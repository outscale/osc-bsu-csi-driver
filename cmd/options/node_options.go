...
func (s *NodeOptions) AddFlags(fs *flag.FlagSet) {
	fs.Var(cliflag.NewStringSlice(&s.LuksOpenFlags), "luks-open-flags", "Flag to add to cryptsetup luksOpen. It may be added multiple time to add multiple flags: '--luks-open-flags=--perf-no_read_workqueue --luks-open-flags=--perf-no_write_workqueue'")
}
