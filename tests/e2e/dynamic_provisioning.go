...
var _ = Describe("[bsu-csi-e2e] [single-az] Dynamic Provisioning", func() {
				...
				Volumes: []testsuites.VolumeDetails{
					{
						VolumeType: cloud.VolumeTypeIO1,
						FSType:     bsucsidriver.FSTypeExt4,
						IopsPerGB:  strconv.Itoa(cloud.MaxIopsPerGb),
						ClaimSize:  "44Gi",
						VolumeMount: testsuites.VolumeMountDetails{
							NameGenerate:      "test-volume-",
							MountPathGenerate: "/mnt/test-",
				...
				Volumes: []testsuites.VolumeDetails{
					{
						VolumeType: cloud.VolumeTypeIO1,
						FSType:     bsucsidriver.FSTypeExt4,
						IopsPerGB:  strconv.Itoa(cloud.MaxIopsPerGb + 1),
						ClaimSize:  "4Gi",
						VolumeMount: testsuites.VolumeMountDetails{
							NameGenerate:      "test-volume-",
							MountPathGenerate: "/mnt/test-",
	...
	It(fmt.Sprintf("[env] should retain PV with reclaimPolicy %q", v1.PersistentVolumeReclaimRetain), func() {
		...
		cloud, err := cloud.NewCloud(region, cloud.WithoutMetadata())
		if err != nil {
	...
	It("should create a volume, delete it from outside and release the volume", func() {
		...
		oscCloud, err := cloud.NewCloud(os.Getenv(OSC_REGION), cloud.WithoutMetadata())
