...
Only cold volumes (volumes not mounted on a VM) can be resized.

### ControllerModifyVolume

Volume type (volumeType) and IOPS (iopspergb) may be changed using VolumeAttributeClasses. Only cold volumes (volumes not mounted on a VM) can be updated.
