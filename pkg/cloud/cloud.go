...
func (c *cloud) DeleteVolume(ctx context.Context, volumeID string) (bool, error) {
	...
	if c.opts.SynchronousDeletes {
		_, err := c.volumeWatcher.WaitUntil(ctx, volumeID, func(vol *osc.Volume) (bool, error) {
			return false, nil
		})
		if !errors.Is(err, batch.ErrNotFound) {
			return false, fmt.Errorf("check for deletion: %w", err)
		}
	}
	return true, nil
}
...
func (c *cloud) DeleteSnapshot(ctx context.Context, snapshotID string) (success bool, err error) {
	...
	if c.opts.SynchronousDeletes {
		_, err := c.snapshotWatcher.WaitUntil(ctx, snapshotID, func(vol *osc.Snapshot) (bool, error) {
			return false, nil
		})
		if !errors.Is(err, batch.ErrNotFound) {
			return false, fmt.Errorf("wait for deletion: %w", err)
		}
	}
	return true, nil
}
