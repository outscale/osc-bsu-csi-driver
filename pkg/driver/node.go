...
func (s *nodeService) NodeStageVolume(ctx context.Context, req *csi.NodeStageVolumeRequest) (*csi.NodeStageVolumeResponse, error) {
	...
	if ok := s.inFlight.Insert(req); !ok {
	...
	exists, err := s.mounter.ExistsPath(target)
	if err != nil {
	...
	device, _, err := s.mounter.GetDeviceName(target)
	if err != nil {
	...
	if isEncrypted {
		...
		// Check passphrase
		if err := s.mounter.CheckLuksPassphrase(source, passphrase); err != nil {
			msg := fmt.Sprintf("error while checking passphrase for %v, err: %v", volumeID, err)
			return nil, status.Error(codes.Internal, msg)
		}

		// Open disk
		if _, err := s.mounter.LuksOpen(source, encryptedDeviceName, passphrase, s.driverOptions.luksOpenFlags...); err != nil {
	...
	existingFormat, err := s.mounter.GetDiskFormat(source)
	if err != nil {
		msg := ""
		if isEncrypted {
			if closeError := s.mounter.LuksClose(encryptedDeviceName); closeError != nil {
	...
	if fsType == FSTypeXfs {
		if existingFormat == "" {
			...
			cmdOut, cmdErr := s.mounter.Command("mkfs.xfs", argsXfs...).CombinedOutput()
			if cmdErr != nil {
	...
	err = s.mounter.FormatAndMount(source, target, fsType, mountOptions)
	if err != nil {
		msg := ""
		if isEncrypted {
			if closeError := s.mounter.LuksClose(encryptedDeviceName); closeError != nil {
...
func (s *nodeService) NodeUnstageVolume(ctx context.Context, req *csi.NodeUnstageVolumeRequest) (*csi.NodeUnstageVolumeResponse, error) {
	...
	dev, refCount, err := s.mounter.GetDeviceName(target)
	if err != nil {
...
func (s *nodeService) NodeExpandVolume(ctx context.Context, req *csi.NodeExpandVolumeRequest) (*csi.NodeExpandVolumeResponse, error) {
	...
	deviceName, _, err := s.mounter.GetDeviceName(volumePath)
	if err != nil {
	...
	devicePath, err := s.findDevicePath(deviceName, volumeID)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "Could not get valid device path for mount path: %q", req.GetVolumePath())
	}

	isLuksMapping, mappingName, err := s.mounter.IsLuksMapping(devicePath)
	if err != nil {
	...
	if isLuksMapping {
		...
		if err := s.mounter.LuksResize(mappingName, passphrase); err != nil {
	...
	r := mountutils.NewResizeFs(s.mounter)
...
func (s *nodeService) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	case *csi.VolumeCapability_Block:
		if err := s.nodePublishVolumeForBlock(ctx, req, mountOptions); err != nil {
			return nil, err
		}
	case *csi.VolumeCapability_Mount:
		if err := s.nodePublishVolumeForFileSystem(ctx, req, mountOptions, mode); err != nil {
			return nil, err
		}
	}
...
func (s *nodeService) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	isMounted, err := s.checkMountTarget(ctx, target)
	if err != nil {
...
func (s *nodeService) isBlockDevice(filename string) (bool, error) {
...
func (s *nodeService) getBlockSizeBytes(devicePath string) (int64, error) {
	cmd := s.mounter.Command("blockdev", "--getsize64", devicePath)
...
func (s *nodeService) nodePublishVolumeForBlock(ctx context.Context, req *csi.NodePublishVolumeRequest, mountOptions []string) error {
	logger := klog.FromContext(ctx)
	target := req.GetTargetPath()
	volumeID := req.GetVolumeId()
	...
	exists, err = s.mounter.ExistsPath(globalMountPath)
	if err != nil {
...
func (s *nodeService) nodePublishVolumeForFileSystem(ctx context.Context, req *csi.NodePublishVolumeRequest, mountOptions []string, mode *csi.VolumeCapability_Mount) error {
	logger := klog.FromContext(ctx)
	target := req.GetTargetPath()
	source := req.GetStagingTargetPath()
...
func (s *nodeService) findDevicePath(devicePath, volumeID string) (string, error) {
...
// doesn't exist.
func (s *nodeService) checkMountTarget(ctx context.Context, target string) (bool, error) {
	logger := klog.FromContext(ctx)
	/*
	...
	*/
	...
	if err != nil && !os.IsNotExist(err) {
		...
		_, pathErr := s.mounter.ExistsPath(target)
