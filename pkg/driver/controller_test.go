...
func TestNewControllerService(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:         "OSC_REGION variable not set, newMetadata does not error",
			newCloudFunc: getNewCloudFunc(testRegion),
		},
		{
			name:                  "OSC_REGION variable not set, newMetadata errors",
			newCloudFunc:          getNewCloudFunc(testRegion),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.region == "" {
				...
				os.Setenv("OSC_REGION", tc.region)
				defer os.Unsetenv("OSC_REGION")
			}
