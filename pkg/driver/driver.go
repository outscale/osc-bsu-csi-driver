...
func (d *Driver) checkTools() error {
	...
	if err != nil {
		return fmt.Errorf("mkfs.ext4 issue: %s", string(out))
	}
	klog.V(3).InfoS(string(out))

	out, err = d.mounter.Command("mkfs.xfs", "-V").CombinedOutput()
	if err != nil {
		return fmt.Errorf("mkfs.xfs issue: %s", string(out))
	}
	klog.V(3).InfoS(string(out))

	out, err = d.mounter.Command("cryptsetup", "-V").CombinedOutput()
	if err != nil {
		return fmt.Errorf("cryptsetup issue: %s", string(out))
	}
	klog.V(3).InfoS(string(out))
...
func (d *Driver) Run() error {
	...
	klog.V(1).InfoS(fmt.Sprintf("Listening for connections on address: %v", listener.Addr()))
